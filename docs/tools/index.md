...
`usage` is the read-only, machine-readable usage tool in the sessions group. It reports token totals, message/request counts, provider quota windows, and optional chunked deltas for the current or another visible session.
